...
namespace BizHawk.Client.EmuHawk
{
	public partial class MainForm : FormBase, IDialogParent, IMainFormForApi, IMainFormForTools, IMainFormForRetroAchievements
	{
		...
		{	
			...
			foreach(var (systemIds, coreNames) in CoreInventory.Instance.SystemGroups.OrderBy(tuple => tuple.SystemIds[0]))
			{
				if (coreNames.Count < 2) continue;

				var displayText = string.Join(" | ", systemIds);
				var submenu = new ToolStripMenuItem { Text = displayText };
